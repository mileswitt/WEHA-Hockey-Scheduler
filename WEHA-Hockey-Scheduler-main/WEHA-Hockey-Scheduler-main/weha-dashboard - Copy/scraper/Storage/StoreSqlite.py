"""
StoreSqlite.py — Scrapy item pipeline that writes scraped data into the
project's local SQLite database (server/data/weha.db).

Replaces the original StoreMysql.py which targeted a remote MySQL/TiDB instance.
...
# Path from scraper/ → ../server/data/weha.db
...
class SqliteStorage:
    def open_spider(self, spider=None):
        # SQLite is safe for multi-threaded use in WAL mode with check_same_thread=False
        self.lock = threading.Lock()
        self.conn = sqlite3.connect(DB_PATH, check_same_thread=False)
        self.conn.execute("PRAGMA journal_mode=WAL")
        self.conn.execute("PRAGMA foreign_keys=ON")
        # Add Ppg column if this is an older database that predates the column
        try:
            self.conn.execute("ALTER TABLE Player ADD COLUMN Ppg REAL")
            self.conn.commit()
        except Exception:
