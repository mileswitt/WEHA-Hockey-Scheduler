// Public team detail page — "/team/:teamID".
// Fetches team info and its roster in parallel on mount, then renders a header
// card with standings stats and a roster section with a grid/list view toggle.
// All data comes from two endpoints: /api/localteams (full team list, filtered
// client-side by TeamID) and /api/players/:id (roster for that team).
...
export default function TeamPage() {
  ...
  // 'grid' shows player avatar cards, 'list' shows a compact row layout
  const [view,    setView]    = useState('grid')

  // Fetch team metadata and roster in parallel so both are available at the same time
  useEffect(() => {
    if (!teamID) return
  ...
  // Win percentage — guard against divide-by-zero for teams with no games played
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0f2b46', fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '32px 24px' }}>
        ...
        {/* Team header card — white card with name, league badge, and stat row */}
        <div style={{
          background:   '#fff',
          borderRadius: 12,
          padding:      '28px 32px',
          ...
          {/* Standings stat strip — renders each stat label/value pair from an array
              so adding or reordering stats doesn't require changing JSX structure */}
        ...
        <div style={{
          ...
          {/* Header row with player count and grid/list toggle */}
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
            ...
            {/* Pill toggle — active view gets a white background inside the dark pill */}
            <div style={{
              display:      'flex',
              background:   '#111',
              borderRadius: 20,
          ...
          {/* Player cards — same data renders differently depending on view mode */}
          {players.length === 0 ? (
            ...
            <div style={{
              ...
              {players.map(player => (
                <div key={player.PlayerID} style={{
                  ...
                  {/* Avatar circle — shows jersey number or "?" if unknown */}
                  <div style={{
                  ...
                  {/* Player name — additional stats only shown in list view */}
