...
from pulp import LpProblem, LpBinary, LpVariable, LpMinimize, lpSum, value
import random
...
def schedule_division(teams):
    ...
    # create binary decision variables for each game/week/slot combination
    x = {(g, w, s): LpVariable(f"game_{i}_{w}_{s}", cat=LpBinary)
         for i, g in enumerate(hockey_games)
    ...
    for w in range(NUM_WEEKS):
        for s in range(ICE_SLOTS):
            model += lpSum(x[g, w, s] for g in hockey_games) <= 1

    # minimize nothing - just find a feasible schedule
    model += 0
    model.solve()
    ...
    for w in range(NUM_WEEKS):
        for s in range(ICE_SLOTS):
            for g in hockey_games:
                if value(x[g, w, s]) and value(x[g, w, s]) > 0.5:
                    schedule.append({
                        "week": w + 1,
                        "slot": s + 1,
                        "home": g[0],
