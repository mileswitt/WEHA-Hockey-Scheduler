...
export default function TeamPage() {
  ...
  const [players, setPlayers] = useState([])  // ← fixed: was missing
  ...
  useEffect(() => {
    ...
    .then(([teams, players]) => {
      ...
      setTeam(foundTeam)
      setPlayers(players)   // ← fixed: was setPlayers(players) on wrong variable
      setLoading(false)
    })
  ...
  if (loading) return (
    <div style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <p style={{ color: '#666', fontSize: 18 }}>Loading team...</p>
    </div>
  )
