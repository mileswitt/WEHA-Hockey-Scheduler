// Reseed Database page — triggers the Python web scraper that pulls the latest
// data from the Gamesheet API and writes it into the local database.
//
// Because the scrape can take several minutes, the backend runs it as a
// background process and exposes a /api/webScrapeStatus endpoint. This page
// polls that endpoint every 20 seconds while the scraper is running, streaming
// log lines into the UI as they arrive. If the admin refreshes mid-scrape,
// the mount effect re-connects to the running job automatically.
//
// The run button requires a valid auth token (passed as a header) because the
// scraper can modify the database — unauthenticated callers would be blocked.
...
export default function WebScraper() {
  ...
  // Fetches the current scraper status and appends any new log lines.
  // Network blips are silently swallowed — polling will retry on the next tick.
  ...
  // On mount, check if a scrape is already running (e.g. admin reloaded the page
  // mid-scrape) and resume polling automatically if so.
  useEffect(() => {
  ...
  async function handleCancel() {
    try {
      const res  = await fetch('/api/cancelWebScrape', {
        method:  'POST',
      ...
      // ignore — the close handler on the server will still mark it stopped
    }
  }
  ...
  // Derive display state from the raw status object so the JSX stays declarative
  ...
  // Detect auth/access errors specifically so we can show a more helpful message
  // about expired Gamesheet credentials rather than just a generic failure
      ...
      <div className="flex items-center gap-4 mb-4">
        ...
        {isRunning && (
          <button
            ...
            className="px-4 py-3 rounded font-semibold text-white bg-gray-700 hover:bg-gray-600 transition cursor-pointer"
          >
            Cancel
          </button>
        )}
