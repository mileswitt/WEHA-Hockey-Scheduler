...
function PlayerPanel() {
  ...
  const openPlayerModal = async (player) => {
    ...
    try {
      const res = await fetch(`/api/player/${player.PlayerID}/team`)
      const data = await res.json()
      setPlayerTeams(Array.isArray(data) ? data : [])
    } catch {
      setPlayerTeams([])
    } finally {
  ...
  const handleDelete = async () => {
    setDeleting(true)
    try {
      const res = await fetch(`/api/player/${deleteID}`, { method: 'DELETE', headers: { [AUTH_HEADER_KEY]: getAuthToken() } })
      const data = await res.json()
    ...
    <>
      ...
      <div className="flex items-center gap-3 mb-3 flex-wrap">
        <div className="relative" ref={searchRef}>
          <input
            value={search}
            onChange={e => { setSearch(e.target.value); setPage(0); setShowDropdown(true) }}
            onKeyDown={handleSearchKeyDown}
            onFocus={() => search.trim() && setShowDropdown(true)}
            onBlur={() => setTimeout(() => setShowDropdown(false), 150)}
            placeholder="Search for players…"
            className="bg-gray-700 border border-gray-600 focus:border-blue-400 text-white rounded px-3 py-1.5 text-sm w-64 focus:outline-none transition"
          />
          {showDropdown && search.trim() && filtered.length > 0 && (
            <div className="absolute top-full left-0 mt-1 w-72 bg-gray-800 border border-gray-600 rounded-lg shadow-xl z-50 max-h-56 overflow-y-auto">
              {filtered.slice(0, 8).map(p => (
                <button
                  key={p.PlayerID}
                  onMouseDown={() => { setShowDropdown(false); openPlayerModal(p) }}
                ...
                >
                  ...
                  <span className="flex items-center gap-1.5 shrink-0">
                    {p.JerseyNumber && <span className="text-gray-400 text-xs">#{p.JerseyNumber}</span>}
              ...
              {filtered.length > 8 && (
                <p className="text-gray-500 text-xs px-3 py-1.5 border-t border-gray-700">{filtered.length - 8} more — keep typing to narrow down</p>
              )}
            </div>
          )}
        </div>
      ...
      {/* Player detail modal */}
      {selectedPlayer && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[9999]" onClick={() => setSelectedPlayer(null)}>
          <div className="bg-gray-800 rounded-xl p-6 w-96 shadow-2xl border border-gray-700" onClick={e => e.stopPropagation()}>
            <div className="flex items-start justify-between mb-4">
              <div>
                <h3 className="text-white text-lg font-semibold">{selectedPlayer.FirstName} {selectedPlayer.LastName}</h3>
                <div className="flex items-center gap-2 mt-1">
                  <span className={`text-xs px-2 py-0.5 rounded font-medium ${selectedPlayer.Position === 'G' ? 'bg-yellow-700 text-yellow-100' : selectedPlayer.Position === 'D' ? 'bg-blue-700 text-blue-100' : 'bg-gray-600 text-gray-200'}`}>
                    {selectedPlayer.Position || 'F'}
                  </span>
                  {selectedPlayer.JerseyNumber && (
                    <span className="text-gray-400 text-sm">#{selectedPlayer.JerseyNumber}</span>
                  )}
                  <span className="text-gray-500 text-xs">ID {selectedPlayer.PlayerID}</span>
                </div>
              </div>
              <button onClick={() => setSelectedPlayer(null)} className="text-gray-500 hover:text-white text-lg leading-none transition">✕</button>
            </div>
            ...
            <div className="grid grid-cols-4 gap-2 mb-4">
              {[
                ...
                { label: 'GP', value: selectedPlayer.GamesPlayed ?? '—' },
                { label: 'GAA', value: selectedPlayer.Gaa != null ? Number(selectedPlayer.Gaa).toFixed(2) : '—' },
              ].map(stat => (
                <div key={stat.label} className="bg-gray-700/60 rounded-lg p-2.5 text-center">
            ...
            <div className="border-t border-gray-700 pt-4">
              <p className="text-gray-400 text-xs uppercase tracking-wide font-semibold mb-2">Teams</p>
              {teamsLoading ? (
                <p className="text-gray-500 text-sm">Loading…</p>
              ) : playerTeams.length === 0 ? (
                <p className="text-gray-500 text-sm">Not assigned to any team.</p>
              ) : (
                <div className="flex flex-col gap-1.5">
                  {playerTeams.map(t => (
                    <button
                      key={t.ScheduledTeamID}
                      onClick={() => { setSelectedPlayer(null); navigate(`/team/${t.ScheduledTeamID}`) }}
                      className="flex items-center justify-between w-full bg-blue-600/20 hover:bg-blue-600/40 border border-blue-600/30 rounded-lg px-3 py-2 transition group"
                    >
                      <span className="text-blue-300 text-sm font-medium group-hover:text-white transition">{t.TeamName}</span>
                      <span className="text-blue-400 text-xs group-hover:text-white transition">View Team →</span>
                    </button>
                  ))}
                </div>
              )}
                ...
                {paged.map(p => (
                  editingID === p.PlayerID ? (
                    ...
                    <tr key={p.PlayerID} className="border-b border-gray-700/50 hover:bg-gray-700/30 transition">
                      ...
                      <td className="px-3 py-2">
                        <button onClick={() => openPlayerModal(p)} className="text-white hover:text-blue-300 transition text-left">
                          {p.FirstName} {p.LastName}
                        </button>
                      </td>
                      <td className="px-3 py-2">
