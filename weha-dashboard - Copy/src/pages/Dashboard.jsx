...
export default function Dashboard({ allTeams, teamsLoaded = true, onDeleteDivision, onDeleteTeam, onUpdateTeams, onAddTeam, onMoveTeam }) {
  ...
  // Committed (scheduled) teams — persisted in ScheduledTeam table
  ...
  // Save generated teams to DB state
  ...
  const handleSaveToDB = async () => {
    ...
    if (!leagueObj) {
      try {
        const r = await fetch('/api/league', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: selectedLeague }) })
    ...
    if (!divisionObj && leagueObj) {
      try {
        const r = await fetch('/api/division', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: selectedDivision, leagueID: leagueObj.id }) })
        const d = await r.json()
  ...
  // Merge: DB leagues take priority for ID lookups; CSV leagues for display names
  ...
  // Build flat team list based on current filter
  const buildTeamList = () => {
    const list = []
  ...
  const seasonsForLeague = dbSeasons.filter(s => s.leagueID === leagueObj?.id)
  ...
  return (
    <div>
      ...
      {/* Teams Section */}
      {teamList.length > 0 || filteredScheduledTeams.length > 0 || creatingTeam ? (
        <div>
          {(teamList.length > 0 || filteredScheduledTeams.length > 0) && (
            <>
                    ...
                    {stEditingId === team.ScheduledTeamID ? (
                      ...
                      <>
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <div className="flex gap-1 shrink-0">
                            <button onClick={() => handleStRenameStart(team)} className="p-1 text-gray-400 hover:text-yellow-400 rounded text-xs transition" title="Rename">✏️</button>
            ...
            <div className="bg-gray-900 border-2 border-yellow-400 rounded-lg overflow-hidden mb-4">
              <div className="bg-yellow-400 px-6 py-3 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-black/70 text-sm">— {editState.league} / {editState.division}</span>
                </div>
                  ...
                  <div className="mb-4 p-3 bg-blue-900/50 border border-blue-500 rounded">
                    ...
                    <div className="flex gap-2 flex-wrap">
                      {teamList.filter(t => t.key !== editingTeamKey).map(t => (
                        <button key={t.key} onClick={() => handleMoveToTeam(t)} className="bg-blue-600 hover:bg-blue-500 text-white text-xs px-3 py-1.5 rounded transition">
                      ...
                      {editState.players.map((player, i) => (
                        <tr key={i} className={`border-b border-gray-700/50 ${movingPlayerIndex === i ? 'bg-blue-900/30' : i % 2 === 0 ? '' : 'bg-gray-800/50'}`}>
                          ...
                          <td className="py-3 pr-4">
                            <input value={player.name} onChange={e => handlePlayerNameChange(i, e.target.value)} className="bg-gray-800 border border-gray-600 focus:border-yellow-400 text-white rounded px-3 py-1.5 text-sm w-40 focus:outline-none transition" />
                          </td>
                          <td className="py-3 pr-4">
                            <span className={`text-xs px-2 py-0.5 rounded ${player.isGoalie || player.position === 'G' ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-300'}`}>
                              {player.isGoalie || player.position === 'G' ? 'G' : (player.position || 'F')}
                          ...
                          <td className="py-3 pr-4">
                            <input type="number" min={0} step={0.1} value={player.skill ?? player.experience ?? 0} onChange={e => handlePlayerExpChange(i, e.target.value)} className="bg-gray-800 border border-gray-600 focus:border-yellow-400 text-white rounded px-3 py-1.5 text-sm w-20 focus:outline-none transition" />
                          </td>
                          <td className="py-3">
                            <div className="flex gap-2">
                              <button onClick={() => setMovingPlayerIndex(movingPlayerIndex === i ? null : i)} className={`text-xs px-3 py-1.5 rounded transition ${movingPlayerIndex === i ? 'bg-blue-500 text-white' : 'bg-gray-700 hover:bg-blue-600 text-gray-300 hover:text-white'}`}>Move</button>
                              <button onClick={() => handleRemovePlayer(i)} className="text-xs bg-gray-700 hover:bg-red-600 text-gray-300 hover:text-white px-3 py-1.5 rounded transition">Remove</button>
                            </div>
                          </td>
                        </tr>
                      ))}
      ...
      {showEditConfirm && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[9999]">
          <div className="bg-gray-800 rounded-lg p-6 w-80 shadow-xl border border-gray-700">
            ...
            <p className="text-gray-400 text-sm mb-6">Save changes to <span className="text-white font-medium">{editState?.teamName}</span>?</p>
            <div className="flex gap-3">
