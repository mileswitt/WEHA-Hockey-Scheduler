...
export default function Upload({ onTeamsGenerated }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res  = await fetch('/api/generate-teams/advanced', { method: 'POST', headers: { [AUTH_HEADER_KEY]: getAuthToken() }, body: formData })
  ...
  const DropZone = ({ label, file, onFile, errorKey }) => {
    ...
    return (
      <div className="flex flex-col gap-1">
        ...
        >
          {dragging ? <p className="text-blue-400">Drop it here!</p>
  ...
  return (
    <div className="w-full max-w-4xl">
      ...
      <div className="bg-gray-800 p-6 rounded-lg mb-6">
        ...
        {leaguesError && <p className="text-red-400 text-sm mb-3">{leaguesError}</p>}
      ...
      <div className={`bg-gray-800 p-6 rounded-lg ${!leaguesLoaded && leagues.length === 0 ? 'opacity-50 pointer-events-none' : ''}`}>
        ...
        {success && (
          <div className="mb-6 p-3 bg-green-700 text-white rounded flex items-center justify-between gap-4">
            <span>✅ Teams generated successfully!</span>
            <Link to="/admin" className="bg-white/20 hover:bg-white/30 text-white text-sm font-medium px-4 py-1.5 rounded transition whitespace-nowrap">
              View Dashboard →
            </Link>
          </div>
        )}
        ...
        <div className="grid grid-cols-2 gap-6 mb-6">
          <div>
            ...
            <p className="text-gray-500 text-xs mt-1.5">Required columns: PlayerID, FirstName, LastName, Position, Goals, Assists, GamesPlayed, Gaa</p>
          </div>
          <div>
            <DropZone label="Player Team CSV (player_team.csv)" file={playerTeamFile} onFile={handlePlayerTeamFile} errorKey="playerTeamFile" />
            <p className="text-gray-500 text-xs mt-1.5">Required columns: PlayerID, TeamID, SeasonID</p>
          </div>
        </div>
