...
// Color map for each division — dark enough to keep white text readable
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Home team name — Bebas Neue at an explicit size where it looks crisp */}
        <p className="text-center leading-tight mb-1"
...
export default function LeagueCalendar() {
  ...
  // Raw game rows kept so we can derive per-league division lists
  ...
  // Divisions that exist within the currently selected league (or all if "All Leagues")
  ...
  // Reset division filter whenever it's no longer valid for the new league
  useEffect(() => {
...
useEffect(() => {
    fetchGames()
      .then(data => {
        ...
        // Build league filter list dynamically from data
  ...
  return (
    ...
    >
      ...
      {/* Division filter buttons — scoped to the active league */}
...
const CALENDAR_CSS = `
  ...
  /* Popover ("+N more" panel) — dark theme so date/event text is visible */
