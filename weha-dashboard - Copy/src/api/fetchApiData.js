...
//
// Cache strategy: game schedule data is cached in localStorage for 10 minutes.
// This avoids re-fetching on every component mount (all three calendar components
// call fetchGames independently) and makes the public pages feel instant on repeat
// visits. Call clearScheduleCache() after publishing a schedule so admins and
// visitors immediately see the latest games rather than stale data.

const CACHE_KEY = 'weha_schedule_v1'
const CACHE_TTL = 30 * 1000 // 30 seconds — server-side cache handles heavier lifting
