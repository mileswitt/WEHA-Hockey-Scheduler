...
// Strip trailing year suffix from league names so "WEHA Fall Adult Leagues 2024"
// and "WEHA Fall Adult Leagues 2025" match as the same base league.
function baseLeagueName(name) {
  return (name || '').replace(/\s+\d{4}(-\d{4})?$/, '').trim()
}

// Returns a player-profile filter function for league matching.
// Primary path: collect the LeagueIDs of the source seasons + all leagues whose base
// name matches the target, then filter by p.leagueID. This is immune to year-suffix
// naming variations. Falls back to name comparison if the DB lookup fails.
async function buildLeagueFilter(targetLeagueName, sourceSeasonIDs) {
  if (!targetLeagueName) return null
  const db = getDb()
  const allowed = new Set()

  // Collect league IDs from source seasons (exact DB lookup — handles cross-year leagues)
  if (sourceSeasonIDs && sourceSeasonIDs.length > 0) {
    try {
      const [rows] = await db.query(
        'SELECT DISTINCT LeagueID FROM Season WHERE SeasonID IN (?)',
        [sourceSeasonIDs.map(Number)]
      )
      rows.forEach(r => allowed.add(Number(r.LeagueID)))
    } catch { /* non-fatal */ }
  }

  // Also collect league IDs whose name (base or exact) matches the target
  try {
    const base = baseLeagueName(targetLeagueName)
    const [leagues] = await db.query('SELECT LeagueID, Name FROM League')
    leagues.forEach(l => {
      if (l.Name === targetLeagueName || baseLeagueName(l.Name) === base) {
        allowed.add(Number(l.LeagueID))
      }
    })
  } catch { /* non-fatal */ }

  if (allowed.size > 0) return (p) => allowed.has(Number(p.leagueID))

  // Final name-only fallback
  const base = baseLeagueName(targetLeagueName)
  return (p) => p.leagueName === targetLeagueName || baseLeagueName(p.leagueName) === base
}
...
// Reference to the active Python child process so it can be cancelled
...
app.get('/api/player-count', async (req, res) => {
  ...
  const divisionName = req.query.division_name || null
  ...
  try {
    let profiles = await buildPlayerProfilesMultiSeason(seasonIDs)
    const leagueFilter = await buildLeagueFilter(leagueName, seasonIDs)
    if (leagueFilter) profiles = profiles.filter(leagueFilter)
    if (divisionName) profiles = profiles.filter(p => p.division === divisionName)
    const dedupMap = new Map()
    for (const p of profiles) {
      const existing = dedupMap.get(p.playerID)
      if (!existing || p.skill > existing.skill) dedupMap.set(p.playerID, p)
    }
    profiles = [...dedupMap.values()]
...
app.post('/api/generate-teams/from-db', requireAuth, async (req, res) => {
  ...
  const leagueFilter = await buildLeagueFilter(leagueName, seasonIDs)
  if (leagueFilter) filtered = filtered.filter(leagueFilter)
  if (divisionName) filtered = filtered.filter(p => p.division === divisionName)

  // Deduplicate by PlayerID — a player may have profiles for multiple year-suffix League
  // records of the same logical league. Keep the highest-skill profile per player.
  const dedupMap = new Map()
  for (const p of filtered) {
    const existing = dedupMap.get(p.playerID)
    if (!existing || p.skill > existing.skill) dedupMap.set(p.playerID, p)
  }
  filtered = [...dedupMap.values()]

  if (filtered.length === 0) {
    const scope = [leagueName, divisionName].filter(Boolean).join(' / ')
    return res.status(400).json({
      error: scope
        ? `No players found for ${scope} in the selected seasons.`
        : 'No players found in the selected seasons.'
    })
  }
...
app.get('/api/data-quality', async (req, res) => {
  ...
  const leagueFilter = await buildLeagueFilter(league_name, season_id ? [season_id] : [])
  if (leagueFilter) profiles = profiles.filter(leagueFilter)
...
app.put('/api/player/:id', requireAuth, async (req, res) => {
...
app.post('/api/season', requireAuth, async (req, res) => {
...
// --- POST /api/league ---
app.post('/api/league', requireAuth, async (req, res) => {
  const { name } = req.body
...
app.post('/api/division', requireAuth, async (req, res) => {
  const { name, leagueID } = req.body
...
app.post('/api/generate-teams/advanced', requireAuth, upload.fields([
...
app.post('/api/generate-teams', requireAuth, upload.single('file'), (req, res) => {
...
app.post('/api/runWebScrape', requireAuth, (_req, res) => {
  ...
  pythonProcess.on('close', code => {
    ...
    } else if (scraperStatus.message === 'Cancelled by user') {
      // already set by cancel endpoint — leave status as-is
    } else {
      scraperStatus.success = false
...
// --- POST /api/cancelWebScrape ---
// Kills the active scraper process if one is running.
// Requires a valid admin JWT token.
app.post('/api/cancelWebScrape', requireAuth, (_req, res) => {
  if (!scraperStatus.running || !activeScraperProcess) {
    return res.json({ scraperStatus })
  }
  ...
  scraperStatus.running = false
  scraperStatus.success = false
  scraperStatus.message = 'Cancelled by user'
  scraperStatus.logs.push('✗ Scraper cancelled by user')
  res.json({ scraperStatus })
})
...
app.post('/api/draft-teams', requireAuth, async (req, res) => {
...
app.delete('/api/draft-teams', requireAuth, async (req, res) => {
  ...
  try {
    const [teams] = await db.query(`
      SELECT st.ScheduledTeamID, st.Name, st.LeagueID, st.DivisionID, st.SeasonID,
             ...
             (SELECT COUNT(*) > 0 FROM Game g
              WHERE (g.HomeTeamID = st.ScheduledTeamID OR g.AwayTeamID = st.ScheduledTeamID)
                AND g.CurrentGameStatus = 'Draft') AS hasDraftGames,
             (SELECT COUNT(*) > 0 FROM Game g
              WHERE (g.HomeTeamID = st.ScheduledTeamID OR g.AwayTeamID = st.ScheduledTeamID)
                AND g.CurrentGameStatus != 'Draft') AS hasPublishedGames
...
app.delete('/api/scheduled-teams/deduplicate', requireAuth, async (_req, res) => {
  const db = getDb()
  try {
    const [rows] = await db.query(`
...
// Returns { deleted: N }
app.delete('/api/scheduled-teams/empty', requireAuth, async (_req, res) => {
  const db = getDb()
  try {
    const [rows] = await db.query(`
...
// --- GET /api/scheduled-teams/carry-over-preview ---
// Returns the saved ScheduledTeams (and their player counts) for a source season,
// so the admin can preview what will be carried over before committing.
// Query: sourceSeasonID, leagueID, [divisionID]
app.get('/api/scheduled-teams/carry-over-preview', async (req, res) => {
  const { sourceSeasonID, leagueID, divisionID, sourceLeagueID } = req.query
  if (!sourceSeasonID || !leagueID) {
    return res.status(400).json({ error: 'sourceSeasonID and leagueID are required.' })
  }
  const db = getDb()
  const effectiveLeagueID = sourceLeagueID ? Number(sourceLeagueID) : Number(leagueID)
  const where  = ['st.SeasonID = ?', 'st.LeagueID = ?']
  const params = [Number(sourceSeasonID), effectiveLeagueID]
  if (divisionID) { where.push('st.DivisionID = ?'); params.push(Number(divisionID)) }
  try {
    const [teams] = await db.query(`
      SELECT st.ScheduledTeamID, st.Name, st.DivisionID,
             d.Name AS DivisionName,
             (SELECT COUNT(*) FROM ScheduledTeamPlayer stp WHERE stp.ScheduledTeamID = st.ScheduledTeamID) AS playerCount
      FROM ScheduledTeam st
      JOIN Division d ON d.DivisionID = st.DivisionID
...
// --- POST /api/scheduled-teams/carry-over ---
// Copies ScheduledTeam rows (and optionally their player assignments) from one
// season into a new target season. Use this to roll existing teams forward without
// regenerating from scratch.
// Body: { sourceSeasonID, targetSeasonID, leagueID, divisionID, keepPlayers }
app.post('/api/scheduled-teams/carry-over', requireAuth, async (req, res) => {
  const { sourceSeasonID, targetSeasonID, leagueID, divisionID, keepPlayers = true, sourceLeagueID } = req.body
  if (!sourceSeasonID || !targetSeasonID || !leagueID || !divisionID) {
    return res.status(400).json({ error: 'sourceSeasonID, targetSeasonID, leagueID, and divisionID are required.' })
  }
  const db = getDb()
  const effectiveSourceLeagueID = sourceLeagueID ? Number(sourceLeagueID) : Number(leagueID)

  const [[{ cnt }]] = await db.query(
    'SELECT COUNT(*) AS cnt FROM ScheduledTeam WHERE LeagueID = ? AND DivisionID = ? AND SeasonID = ?',
    [Number(leagueID), Number(divisionID), Number(targetSeasonID)]
  )
  if (cnt > 0) {
    return res.status(409).json({ error: 'Teams already exist for the target season. Delete them first, then carry over.' })
  }

  const [sourceTeams] = await db.query(
    'SELECT ScheduledTeamID, Name FROM ScheduledTeam WHERE SeasonID = ? AND LeagueID = ? AND DivisionID = ? ORDER BY Name',
    [Number(sourceSeasonID), effectiveSourceLeagueID, Number(divisionID)]
  )
  if (sourceTeams.length === 0) {
    return res.status(404).json({ error: 'No saved teams found for the selected source season and division.' })
  }

  // Guard: if keeping players, check that no goalie from the source teams is
  // already assigned to a team in the target season (prevents the same goalie
  // appearing on multiple teams when multiple carry-overs target the same season).
  if (keepPlayers) {
    const sourceIDs = sourceTeams.map(t => t.ScheduledTeamID)
    const srcPlaceholders = sourceIDs.map(() => '?').join(',')
    const [sourceGoalies] = await db.query(
      `SELECT DISTINCT stp.PlayerID, p.FirstName, p.LastName, st.Name AS SourceTeam
       FROM ScheduledTeamPlayer stp
       JOIN Player p ON p.PlayerID = stp.PlayerID
       JOIN ScheduledTeam st ON st.ScheduledTeamID = stp.ScheduledTeamID
       WHERE stp.ScheduledTeamID IN (${srcPlaceholders}) AND p.Position = 'G'`,
      sourceIDs
    )
    if (sourceGoalies.length > 0) {
      const goalieIDs = sourceGoalies.map(g => g.PlayerID)
      const gPlaceholders = goalieIDs.map(() => '?').join(',')
      const [conflicting] = await db.query(
        `SELECT stp.PlayerID, p.FirstName, p.LastName, st.Name AS ExistingTeam
         FROM ScheduledTeamPlayer stp
         JOIN Player p ON p.PlayerID = stp.PlayerID
         JOIN ScheduledTeam st ON st.ScheduledTeamID = stp.ScheduledTeamID
         WHERE st.SeasonID = ? AND stp.PlayerID IN (${gPlaceholders})`,
        [Number(targetSeasonID), ...goalieIDs]
      )
      if (conflicting.length > 0) {
        const details = conflicting
          .map(g => `"${g.FirstName} ${g.LastName}" is already a goalie on "${g.ExistingTeam}" this season`)
          .join('; ')
        return res.status(409).json({
          error: `Cannot carry over: goalie(s) already assigned to another team this season. ${details}. Remove the duplicate assignment first, or carry over without players.`,
        })
      }
    }
  }
  ...
  try {
    const carriedTeams = await withTransaction(async conn => {
      const results = []
      for (const team of sourceTeams) {
        const [ins] = await conn.query(
          'INSERT INTO ScheduledTeam (Name, LeagueID, DivisionID, SeasonID, EnteredDate, EnteredTime) VALUES (?, ?, ?, ?, ?, ?)',
          [team.Name, Number(leagueID), Number(divisionID), Number(targetSeasonID), enteredDate, enteredTime]
        )
        const newID = ins.insertId
        let playerCount = 0
        if (keepPlayers) {
          const [players] = await conn.query(
            'SELECT PlayerID FROM ScheduledTeamPlayer WHERE ScheduledTeamID = ?',
            [team.ScheduledTeamID]
          )
          for (const { PlayerID } of players) {
            await conn.query(
              'INSERT IGNORE INTO ScheduledTeamPlayer (ScheduledTeamID, PlayerID) VALUES (?, ?)',
              [newID, PlayerID]
            )
          }
          playerCount = players.length
        }
        results.push({ scheduledTeamID: newID, name: team.Name, playerCount })
      }
      return results
    })
    ...
    res.json({
      ok:                true,
      teamsCarriedOver:  carriedTeams.length,
      playersCarriedOver: carriedTeams.reduce((s, t) => s + t.playerCount, 0),
      teams:             carriedTeams,
    })
  } catch (e) {
    res.status(500).json({ error: e.message })
...
// --- GET /api/gamesheet-teams/preview ---
// Returns scraped teams (Team + PlayerTeam tables) for a given season so the admin
// can preview what Gamesheet data will be carried over before committing.
// Query: sourceSeasonID, leagueID, [divisionID]
app.get('/api/gamesheet-teams/preview', async (req, res) => {
  const { sourceSeasonID, leagueID, divisionID, sourceLeagueID } = req.query
  if (!sourceSeasonID || !leagueID) {
    return res.status(400).json({ error: 'sourceSeasonID and leagueID are required.' })
  }
  const db = getDb()
  const effectiveLeagueID = sourceLeagueID ? Number(sourceLeagueID) : Number(leagueID)
  const where  = ['pt.SeasonID = ?', 't.LeagueID = ?']
  const params = [Number(sourceSeasonID), effectiveLeagueID]
  if (divisionID) { where.push('t.DivisionID = ?'); params.push(Number(divisionID)) }
  try {
    const [teams] = await db.query(`
      SELECT DISTINCT t.TeamID, t.Name, t.DivisionID,
             d.Name AS DivisionName,
             (SELECT COUNT(*) FROM PlayerTeam pt2 WHERE pt2.TeamID = t.TeamID AND pt2.SeasonID = ?) AS playerCount
      FROM Team t
      JOIN PlayerTeam pt ON pt.TeamID = t.TeamID
      LEFT JOIN Division d ON d.DivisionID = t.DivisionID
      WHERE ${where.join(' AND ')}
      ORDER BY t.Name
    `, [Number(sourceSeasonID), ...params])
    res.json({ teams, total: teams.length })
  } catch (e) {
    res.status(500).json({ error: e.message })
  }
...
// --- POST /api/scheduled-teams/carry-over-from-gamesheet ---
// Creates ScheduledTeam rows (and optionally ScheduledTeamPlayer rows) sourced
// from the scraped Gamesheet Team/PlayerTeam tables rather than previously saved
// ScheduledTeam rows.
// Body: { sourceSeasonID, targetSeasonID, leagueID, divisionID, keepPlayers }
app.post('/api/scheduled-teams/carry-over-from-gamesheet', requireAuth, async (req, res) => {
  const { sourceSeasonID, targetSeasonID, leagueID, divisionID, keepPlayers = true, sourceLeagueID } = req.body
  if (!sourceSeasonID || !targetSeasonID || !leagueID || !divisionID) {
    return res.status(400).json({ error: 'sourceSeasonID, targetSeasonID, leagueID, and divisionID are required.' })
  }
  ...
  const [sourceTeams] = await db.query(
    `SELECT DISTINCT t.TeamID, t.Name
     FROM Team t
     JOIN PlayerTeam pt ON pt.TeamID = t.TeamID
     WHERE pt.SeasonID = ? AND t.LeagueID = ?
     ORDER BY t.Name`,
    [Number(sourceSeasonID), effectiveSourceLeagueID]
  )
  if (sourceTeams.length === 0) {
    return res.status(404).json({ error: 'No Gamesheet teams found for the selected source season and league. Run the web scraper first to populate the database.' })
  }

  if (keepPlayers) {
    const sourceTeamIDs = sourceTeams.map(t => t.TeamID)
    const srcPlaceholders = sourceTeamIDs.map(() => '?').join(',')
    const [sourceGoalies] = await db.query(
      `SELECT DISTINCT pt.PlayerID, p.FirstName, p.LastName, t.Name AS SourceTeam
       FROM PlayerTeam pt
       JOIN Player p ON p.PlayerID = pt.PlayerID
       JOIN Team t ON t.TeamID = pt.TeamID
       WHERE pt.TeamID IN (${srcPlaceholders}) AND pt.SeasonID = ? AND p.Position = 'G'`,
      [...sourceTeamIDs, Number(sourceSeasonID)]
    )
    if (sourceGoalies.length > 0) {
      const goalieIDs = sourceGoalies.map(g => g.PlayerID)
      const gPlaceholders = goalieIDs.map(() => '?').join(',')
  ...
  try {
    const carriedTeams = await withTransaction(async conn => {
      ...
      for (const team of sourceTeams) {
        ...
        if (keepPlayers) {
          const [players] = await conn.query(
            'SELECT PlayerID FROM PlayerTeam WHERE TeamID = ? AND SeasonID = ?',
            [team.TeamID, Number(sourceSeasonID)]
          )
          for (const { PlayerID } of players) {
            await conn.query(
              'INSERT IGNORE INTO ScheduledTeamPlayer (ScheduledTeamID, PlayerID) VALUES (?, ?)',
...
app.post('/api/scheduled-teams', requireAuth, async (req, res) => {
  ...
  // Guard: each goalie can only appear on one team within this batch
  const goalieMap = new Map()
  const dupeGoalies = []
  for (const t of teams) {
    if (Array.isArray(t.players)) {
      for (const p of t.players) {
        if ((p.isGoalie || p.position === 'G') && p.playerID) {
          const pid = Number(p.playerID)
          if (goalieMap.has(pid)) {
            dupeGoalies.push({
              name: p.name || `Player #${pid}`,
              teams: [goalieMap.get(pid), t.teamName || 'unnamed team'],
            })
          } else {
            goalieMap.set(pid, t.teamName || 'unnamed team')
          }
        }
      }
    }
  }
  if (dupeGoalies.length > 0) {
    const details = dupeGoalies.map(d => `"${d.name}" is on both "${d.teams[0]}" and "${d.teams[1]}"`).join('; ')
    return res.status(409).json({
      error: `Cannot save: duplicate goalie(s) found across teams. Each goalie can only be on one team. ${details}. Remove the duplicate before saving.`,
    })
  }

  // Guard: cross-batch — check if any of these goalies are already on a team
  // in a different division/batch for the same season
  if (goalieMap.size > 0) {
    const goalieIDs = [...goalieMap.keys()]
    const [existingRows] = await db.query(
      `SELECT stp.PlayerID, p.FirstName, p.LastName, st.Name AS TeamName
       FROM ScheduledTeamPlayer stp
       JOIN Player p          ON p.PlayerID  = stp.PlayerID
       JOIN ScheduledTeam st  ON st.ScheduledTeamID = stp.ScheduledTeamID
       WHERE st.SeasonID = ? AND stp.PlayerID IN (${goalieIDs.map(() => '?').join(',')})`,
      [seasonID, ...goalieIDs]
    )
    if (existingRows.length > 0) {
      const crossBatch = existingRows.map(r => {
        const incoming = goalieMap.get(r.PlayerID)
        return `"${r.FirstName} ${r.LastName}" is already a goalie on "${r.TeamName}" this season (incoming team: "${incoming}")`
      })
      return res.status(409).json({
        error: `Cannot save: goalie(s) already assigned to a team this season. ${crossBatch.join('; ')}. Remove the duplicate before saving.`,
      })
    }
  }
...
// --- GET /api/player/:playerID/team ---
// Returns the ScheduledTeam(s) a player currently belongs to.
app.get('/api/player/:playerID/team', async (req, res) => {
  const id = Number(req.params.playerID)
  if (isNaN(id)) return res.status(400).json({ error: 'Invalid ID.' })
  try {
    const [rows] = await getDb().query(`
      SELECT st.ScheduledTeamID, st.Name AS TeamName, st.LeagueID, st.DivisionID
      FROM ScheduledTeamPlayer stp
      JOIN ScheduledTeam st ON st.ScheduledTeamID = stp.ScheduledTeamID
      WHERE stp.PlayerID = ?
      ORDER BY st.ScheduledTeamID DESC
      LIMIT 5
    `, [id])
    res.json(rows)
  } catch (e) {
...
// Rename a scheduled team and/or replace its player roster.
// Body: { name?, playerIDs? }
app.put('/api/scheduled-teams/:id', requireAuth, async (req, res) => {
  const id = Number(req.params.id)
  if (isNaN(id)) return res.status(400).json({ error: 'Invalid ID.' })
    ...
    if (Array.isArray(playerIDs)) {
      await withTransaction(async conn => {
        ...
        for (const pid of playerIDs) {
          await conn.query('INSERT IGNORE INTO ScheduledTeamPlayer (ScheduledTeamID, PlayerID) VALUES (?, ?)', [id, Number(pid)])
        }
      })
    }
    res.json({ ok: true })
...
app.delete('/api/scheduled-teams/:id', requireAuth, async (req, res) => {
  const id = Number(req.params.id)
  if (isNaN(id)) return res.status(400).json({ error: 'Invalid ID.' })
  try {
...
app.post('/api/generate-external/:divisionID', requireAuth, async (req, res) => {
  try {
...
app.delete('/api/games/by-date', requireAuth, async (req, res) => {
...
app.post('/api/games/publish', requireAuth, async (req, res) => {
  const db = getDb()
...
app.post('/api/games/unpublish', requireAuth, async (req, res) => {
  const db = getDb()
  const { gameIDs, divisionID, seasonID } = req.body
  if (Array.isArray(gameIDs) && gameIDs.length > 0) {
...
// --- GET /api/getSeasons ---
// Returns seasons that have at least one ScheduledTeam entry. Game dates come
// from non-Draft games only (used to detect the current/active season). Seasons
// with no games yet still appear so their teams are visible in standings.
app.get('/api/getSeasons', async (_req, res) => {
  try {
    const [rows] = await getDb().query(`
      SELECT s.SeasonID, s.LeagueID, l.Name AS LeagueName, s.Name AS SeasonName,
        MIN(g.GameDate) AS FirstGameDate,
        MAX(g.GameDate) AS LastGameDate
      FROM Season s
      JOIN League l ON l.LeagueID = s.LeagueID
      LEFT JOIN Game g ON g.SeasonID = s.SeasonID AND g.CurrentGameStatus != 'Draft'
      WHERE EXISTS (
        SELECT 1 FROM ScheduledTeam st WHERE st.SeasonID = s.SeasonID
      )
      GROUP BY s.SeasonID, s.LeagueID, l.Name, s.Name
      ORDER BY l.Name ASC, MAX(g.GameDate) DESC
    `)
    res.json(rows)
  } catch (e) {
    res.status(500).json({ error: e.message })
...
const SCHEDULE_QUERY = `
  WITH TeamRecord AS (
    ...
    LEFT JOIN Game g
     ...
     AND g.CurrentGameStatus = 'Completed'
  ...
  WHERE g.CurrentGameStatus IN ('Scheduled', 'Completed', 'Postponed', 'Cancelled')
...
// --- GET /api/standings ---
// Public-facing: returns W/L/T/Pts standings computed from Completed games.
// Optional query params: seasonID, divisionID, leagueID
app.get('/api/standings', async (req, res) => {
  const { seasonID, divisionID, leagueID } = req.query
  const where = []
  const params = []
  if (seasonID)   { where.push('st.SeasonID = ?');   params.push(Number(seasonID)) }
  if (divisionID) { where.push('st.DivisionID = ?'); params.push(Number(divisionID)) }
  ...
  try {
    const [rows] = await getDb().query(`
      SELECT
        ...
        l.LeagueID, l.Name AS LeagueName,
        d.DivisionID, d.Name AS DivisionName,
        s.SeasonID, s.Name AS SeasonName,
        COALESCE(SUM(CASE
          WHEN (g.HomeTeamID = st.ScheduledTeamID AND g.HomeTeamScore > g.AwayTeamScore)
            OR (g.AwayTeamID = st.ScheduledTeamID AND g.AwayTeamScore > g.HomeTeamScore)
      ...
      ${whereClause}
      GROUP BY st.ScheduledTeamID
      ORDER BY l.Name ASC, d.Name ASC, Wins DESC, Ties DESC, Losses ASC, st.Name ASC
    `, params)
    res.json(rows)
  } catch (e) {
    res.status(500).json({ error: e.message })
...
// --- GET /api/players ---
// Search all players by name. ?q=term returns up to 50 matches.
app.get('/api/players', async (req, res) => {
  ...
  try {
    ...
    if (q && q.trim()) {
      const term = `%${q.trim()}%`
      ;[rows] = await db.query(
        `SELECT PlayerID, FirstName, LastName, Position FROM Player
      ...
      ;[rows] = await db.query(
        'SELECT PlayerID, FirstName, LastName, Position FROM Player ORDER BY LastName, FirstName LIMIT 50'
      )
    }
    res.json(rows)
  } catch (e) {
...
app.get('/api/presets', async (_req, res) => {
  try {
    const [rows] = await getDb().query('SELECT * FROM SchedulePreset ORDER BY CreatedAt ASC')
    res.json(rows.map(r => ({
      id:             r.PresetID,
      name:           r.Name,
      startDate:      r.StartDate,
      endDate:        r.EndDate,
      gameDays:       JSON.parse(r.GameDays),
      gameTimes:      JSON.parse(r.GameTimes),
      rounds:         r.Rounds,
...
app.post('/api/presets', requireAuth, async (req, res) => {
  const { name, startDate, endDate, gameDays, gameTimes, rounds, maxGamesPerDay, rink } = req.body
  if (!name || !gameDays?.length || !gameTimes?.length)
    return res.status(400).json({ error: 'name, gameDays, and gameTimes are required.' })
  try {
    const [result] = await getDb().query(
      'INSERT INTO SchedulePreset (Name, StartDate, EndDate, GameDays, GameTimes, Rounds, MaxGamesPerDay, Rink) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
      [name, startDate || null, endDate || null, JSON.stringify(gameDays), JSON.stringify(gameTimes), rounds || 1, maxGamesPerDay || 1, rink || null]
    )
...
app.put('/api/presets/:id', requireAuth, async (req, res) => {
  ...
  try {
    await getDb().query(
      'UPDATE SchedulePreset SET Name=?, StartDate=?, EndDate=?, GameDays=?, GameTimes=?, Rounds=?, MaxGamesPerDay=?, Rink=? WHERE PresetID=?',
      [name, startDate || null, endDate || null, JSON.stringify(gameDays), JSON.stringify(gameTimes), rounds || 1, maxGamesPerDay || 1, rink || null, id]
    )
    res.json({ ok: true })
  } catch (e) { res.status(500).json({ error: e.message }) }
})
...
app.delete('/api/presets/:id', requireAuth, async (req, res) => {
  ...
  if (isNaN(id)) return res.status(400).json({ error: 'Invalid ID.' })
  try {
    await getDb().query('DELETE FROM SchedulePreset WHERE PresetID = ?', [id])
    res.json({ ok: true })
  } catch (e) { res.status(500).json({ error: e.message }) }
})

// ═══════════════════════════════════════════════════════════════════════════════
// SEASON RESET — wipes team/game data so a new season can be built from scratch
// ═══════════════════════════════════════════════════════════════════════════════

// Shared helper: build the list of ScheduledTeamIDs to target for a reset scope.
// When includeDivisions is true the scope expands to ALL teams in those divisions
// (ignoring seasonID) so FK constraints don't block the Division deletes.
async function resolveResetTeamIDs(db, leagueID, seasonID, includeDivisions) {
  if (includeDivisions && leagueID) {
    const [rows] = await db.query(
      'SELECT ScheduledTeamID FROM ScheduledTeam WHERE LeagueID = ?', [leagueID]
    )
    return rows.map(r => r.ScheduledTeamID)
  }
  if (includeDivisions && !leagueID) {
    const [rows] = await db.query('SELECT ScheduledTeamID FROM ScheduledTeam')
    return rows.map(r => r.ScheduledTeamID)
  }
  const where = []
  const params = []
  if (leagueID) { where.push('LeagueID = ?'); params.push(leagueID) }
  if (seasonID) { where.push('SeasonID = ?'); params.push(seasonID) }
  const clause = where.length ? `WHERE ${where.join(' AND ')}` : ''
  const [rows] = await db.query(`SELECT ScheduledTeamID FROM ScheduledTeam ${clause}`, params)
  return rows.map(r => r.ScheduledTeamID)
}

// --- GET /api/season-reset/preview ---
// Returns counts of records that would be deleted — no data is changed.
// Query: ?leagueID=&seasonID=&includeDivisions=true
app.get('/api/season-reset/preview', requireAuth, async (req, res) => {
  ...
  const db = getDb()
  try {
    const [[{ draftCount }]] = await db.query('SELECT COUNT(*) AS draftCount FROM DraftTeam')

    const teamIDs = await resolveResetTeamIDs(db, leagueID, seasonID, includeDivisions)
    const teamCount = teamIDs.length

    let gameCount = 0
    if (teamIDs.length > 0) {
      const [[{ gc }]] = await db.query(
        'SELECT COUNT(*) AS gc FROM Game WHERE HomeTeamID IN (?) OR AwayTeamID IN (?)',
        [teamIDs, teamIDs]
      )
      gameCount = gc
    }
    ...
    let divisionCount = 0
    if (includeDivisions) {
      const divWhere  = leagueID ? 'WHERE LeagueID = ?' : ''
      const divParams = leagueID ? [leagueID] : []
      const [[{ dc }]] = await db.query(`SELECT COUNT(*) AS dc FROM Division ${divWhere}`, divParams)
      divisionCount = dc
    }

    res.json({ draftCount, teamCount, gameCount, divisionCount })
  } catch (e) {
    res.status(500).json({ error: e.message })
  }
})
...
// Permanently deletes all draft teams, the scoped scheduled teams + games,
// and optionally the Division records for the selected league.
// Body: { leagueID?, seasonID?, includeDivisions }
app.post('/api/season-reset', requireAuth, async (req, res) => {
  ...
  const includeDivisions = !!req.body.includeDivisions
  const db = getDb()
  try {
    let draftCount = 0
    if (leagueID) {
      const [[league]] = await db.query('SELECT Name FROM League WHERE LeagueID = ?', [leagueID])
      if (league) {
        const [[{ cnt }]] = await db.query('SELECT COUNT(*) AS cnt FROM DraftTeam WHERE LeagueName = ?', [league.Name])
        draftCount = cnt
        await db.query('DELETE FROM DraftTeam WHERE LeagueName = ?', [league.Name])
      }
    } else {
      const [[{ cnt }]] = await db.query('SELECT COUNT(*) AS cnt FROM DraftTeam')
      draftCount = cnt
      await db.query('DELETE FROM DraftTeam')
    }
    ...
    await withTransaction(async conn => {
      if (teamIDs.length > 0) {
        const [teamRows] = await conn.query(
          'SELECT ScheduledTeamID, Name, LeagueID, DivisionID, SeasonID FROM ScheduledTeam WHERE ScheduledTeamID IN (?)',
          [teamIDs]
        )
        for (const team of teamRows) {
          await logDelete(conn, 'ScheduledTeam', team.ScheduledTeamID, team.Name, {
            LeagueID: team.LeagueID, DivisionID: team.DivisionID, SeasonID: team.SeasonID, reason: 'season_reset',
          })
          const [gr] = await conn.query(
            'DELETE FROM Game WHERE HomeTeamID = ? OR AwayTeamID = ?',
            [team.ScheduledTeamID, team.ScheduledTeamID]
          )
          gamesDeleted += gr.affectedRows
          await conn.query('DELETE FROM ScheduledTeamPlayer WHERE ScheduledTeamID = ?', [team.ScheduledTeamID])
          await conn.query('DELETE FROM ScheduledTeam WHERE ScheduledTeamID = ?', [team.ScheduledTeamID])
          teamsDeleted++
        }
      }
      ...
      if (includeDivisions) {
        ...
        const [divRows] = await conn.query(
          `SELECT DivisionID, Name, LeagueID FROM Division ${divWhere}`, divParams
        )
        for (const div of divRows) {
          await logDelete(conn, 'Division', div.DivisionID, div.Name, {
            LeagueID: div.LeagueID, reason: 'season_reset',
          })
        }
        const [divRes] = await conn.query(`DELETE FROM Division ${divWhere}`, divParams)
        divisionsDeleted = divRes.affectedRows
      }
    })

    invalidateScheduleCache()
    res.json({ ok: true, draftTeamsDeleted: draftCount, teamsDeleted, gamesDeleted, divisionsDeleted })
  } catch (e) {
    res.status(500).json({ error: e.message })
  }
})

// SPA catch-all — serves index.html for any unmatched route so React Router handles routing
app.use((req, res) => {
