...
CREATE TABLE IF NOT EXISTS SchedulePreset (
    ...
    GameTimes      TEXT NOT NULL,
    Rounds         INT NOT NULL DEFAULT 1,
    MaxGamesPerDay INT NOT NULL DEFAULT 1,
    Rink           VARCHAR(255),
    CreatedAt      DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE INDEX idx_draftteam_league_div ON DraftTeam (LeagueName, DivisionName);
CREATE INDEX idx_game_status          ON Game (CurrentGameStatus);
CREATE INDEX idx_game_home_team       ON Game (HomeTeamID);
CREATE INDEX idx_game_away_team       ON Game (AwayTeamID);
CREATE INDEX idx_game_status_home     ON Game (CurrentGameStatus, HomeTeamID);
CREATE INDEX idx_game_status_away     ON Game (CurrentGameStatus, AwayTeamID);
CREATE INDEX idx_playerteam_season    ON PlayerTeam (SeasonID);
CREATE INDEX idx_playerteam_player    ON PlayerTeam (PlayerID);
CREATE INDEX idx_deletelog_type       ON DeleteLog (EntityType);
CREATE INDEX idx_deletelog_at         ON DeleteLog (DeletedAt)
