...
// Returns the same shape as loadReferenceTables() but reads from SQLite.
// Falls back to CSV if the DB has no data.
export function loadReferenceTablesFromDb(dataDir) {
  try {
    const db = getDb()
    const hasData = db.prepare('SELECT COUNT(*) AS n FROM League').get().n > 0
    if (!hasData) return loadReferenceTables(dataDir)

    const teams     = db.prepare('SELECT * FROM Team').all()
    const divs      = db.prepare('SELECT * FROM Division').all()
    const seasons   = db.prepare('SELECT * FROM Season').all()
    const leagues   = db.prepare('SELECT * FROM League').all()
...
// Returns { leagues, divisions, seasons } from SQLite.
// Falls back to CSV if the DB has no data.
export function getLeaguesAndDivisionsFromDb(dataDir) {
  try {
    const db = getDb()
    const hasData = db.prepare('SELECT COUNT(*) AS n FROM League').get().n > 0
    if (!hasData) {
    ...
    const leagues   = db.prepare('SELECT LeagueID, Name FROM League').all()
    ...
    `).all()
    const seasons = db.prepare('SELECT SeasonID, LeagueID, Name FROM Season').all()
...
function queryPlayerRows(db, seasonIDs) {
  let query = `
    SELECT
  ...
  if (seasonIDs && seasonIDs.length > 0) {
    query += ` WHERE pt.SeasonID IN (${seasonIDs.map(() => '?').join(',')})`
...
export function buildPlayerProfilesFromDb(seasonID) {
  const db   = getDb()
  const rows = queryPlayerRows(db, seasonID ? [seasonID] : null)
...
export function buildPlayerProfilesMultiSeason(seasonIDs) {
  ...
  const db   = getDb()
  const rows = queryPlayerRows(db, seasonIDs)
