...
export async function getLeaguesAndDivisionsFromDb(dataDir) {
  try {
    ...
    // Deduplicate by (name, leagueID) keeping the lowest ID — guards against
    // duplicate rows that can accumulate if UNIQUE constraints are missing.
    const seenDivisions = new Map()
    for (const d of divisions) {
      const key = `${d.LeagueID}|${d.Name.toLowerCase()}`
      if (!seenDivisions.has(key)) seenDivisions.set(key, d)
    }
    const seenSeasons = new Map()
    for (const s of seasons) {
      const key = `${s.LeagueID}|${s.Name.toLowerCase()}`
      if (!seenSeasons.has(key)) seenSeasons.set(key, s)
    }

    return {
      leagues:   leagues.map(l => ({ id: l.LeagueID, name: l.Name })),
      divisions: [...seenDivisions.values()].map(d => ({
        id:         d.DivisionID,
        leagueID:   d.LeagueID,
        name:       d.Name,
        leagueName: d.LeagueName,
      })),
      seasons: [...seenSeasons.values()].map(s => ({ id: s.SeasonID, leagueID: s.LeagueID, name: s.Name })),
    }
  } catch {
